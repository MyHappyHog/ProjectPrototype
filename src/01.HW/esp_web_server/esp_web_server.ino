 ...
 *  EEPROM에서 ssid를 읽어들이는 함수
 */
void write_ssid_to_eeprom( String _ssid ) {
  // eeprom 에다가 ssid를 씀.
  // 가장 첫 주소의 값은 ssid 문장의 길이를 나타냄.
...
String read_password_from_eeprom( int firstAddress ) {
  // eeprom 에서 password를 읽어옴.
  // 가장 첫 주소의 값은 password 문장의 길이를 나타냄.
  // 읽은 데이터를 String object로 리턴 함.
}
 ...
 *  EEPROM에서 password를 읽어들이는 함수
 */
void write_password_to_eeprom( String _password ) {
  // eeprom 에다가 password를 씀.
  // 가장 첫 주소의 값은 password 문장의 길이를 나타냄.
  // 뒤에 공간은 남지만 ssid와 맞추기위해서 99 자리수의 제한을 둘 수 있도록...
  // 100 자리 이상일 경우에는 업데이트하지 않고 함수 종료
}

/* 
 *  EEPROM에서 length만큼 읽어들이는 함수
 */
